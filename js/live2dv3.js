(function () {
  ...
  class L2D {
    ...
    load (folder, name, v, bg='assets/res/basic/scene/bg/kanban/green.png') {
      if (!this.models[name]) {
  ...
  class Live2dV3 { // eslint-disable-line no-unused-vars
    constructor ({folderName, basePath, modelName, width = 500, height = 300, bg='assets/res/basic/scene/bg/kanban/green.png', el, sizeLimit, mobileLimit, sounds }) {
      ...
      window.onresize = (event) => {
        ...
        if (this.model) {
          ...
          this.model.position = new PIXI.Point(document.getElementById('posx').value, document.getElementById('posy').value)
        }
      }
      this.isClick = false
          ...
          if(mmotions.FileReferences.Motions[i][0].Sound) {
            ...
            if(subtitleJSON[this.folderName]) {
              if(subtitleJSON[this.folderName][motionId]) {
                ...
                subtitleElement.innerHTML = subtitleJSON[this.folderName][motionId];
  ...
  //custom modification
  changeModel = function(basePath, folderName, modelName, l2dViewer, bg='assets/res/basic/scene/bg/kanban/green.png', x, y) {
